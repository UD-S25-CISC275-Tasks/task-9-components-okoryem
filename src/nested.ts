...
export function sumPoints(questions: Question[]): number {
    return questions.map((ques: Question): number => ques.points).reduce((acc, num) => acc + num, 0);
}
...
export function toCSV(questions: Question[]): string {
    let quesCSV = questions.map((ques: Question): string => `${ques.id},${ques.name},${ques.options.length},${ques.points},${ques.published}`).join("\n");
...
export function makeAnswers(questions: Question[]): Answer[] {
    return questions.map((ques: Question): Answer => ({questionId: ques.id, text: "", submitted: false, correct: false}));
}
...
export function publishAll(questions: Question[]): Question[] {
    return questions.map((ques: Question): Question => ({...ques, options:[...ques.options], published: true}));
}
...
export function sameType(questions: Question[]): boolean {
    let arr = (questions.filter(ques => ques.type === "multiple_choice_question"));
...
export function renameQuestionById(
    ...
    return questions.map((ques: Question): Question => ques.id !== targetId ? { ...ques, options: [...ques.options] } : { ...ques, options: [...ques.options], name : newName});
}
